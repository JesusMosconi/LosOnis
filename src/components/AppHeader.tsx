...
export async function AppHeader(){const session=await getSession();return <header className="app-header"><div className="header-brand"><UserMenu nombre={session?.nombre??""}/><Link href="/pedidos">Los Onis</Link></div><Link aria-label="Ir al calendario" className="today" href="/calendario"><span className="material-symbols-outlined">calendar_today</span></Link></header>}
